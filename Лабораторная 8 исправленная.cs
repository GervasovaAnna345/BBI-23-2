...
class Task1 : Task
{
    ...
    private void Frequency(string text)
    {
        ...
        for (int i = 0; i < letterFrqsy.Length; i++)
        {
            ...
            result += $"Частота буквы {(char)letterFrqsy[i]} составляет {(cnt / (text.Length - pattern.Length)) * 100}%" + "\n";
            cnt = 0;
        }
    }
...
class Task6 : Task
{
    ...
    public override void ParseText(string text)
    {
        ...
        Console.WriteLine("Введите необходимое количество слогов");
    ...
    public override string ToString()
    {
        return "Таких слов в тексте " + res;
    }
}
class Task12 : Task
{
    ...
    public override void ParseText(string text)
    {
        wordCodes =
        [
            new WordCodes("древесины", "**"),
            new WordCodes("многих", "*@"),
            new WordCodes("разрушения", "@*"),
            new WordCodes("движение", "**"),
            new WordCodes("расширению", "@@"),
            new WordCodes("себя", "&*"),
            new WordCodes("дефолта", "*&"),
            new WordCodes("международных", "&&"),
            new WordCodes("финансовой", "@&"),
            new WordCodes("Фьорды", "&@"),
            new WordCodes("окруженные", "%$"),
            new WordCodes("продолжая", "$%"),
            new WordCodes("древесины", "%%"),
            new WordCodes("древесины", "$$"),
            new WordCodes("древесины", "++"),
            new WordCodes("and", "+*"),
            new WordCodes("around", "*+"),
            new WordCodes("the", "=#"),
            new WordCodes("путешествие", "#="),
            new WordCodes("круглую", "##"),
            new WordCodes("значение", "=="),
        ];
