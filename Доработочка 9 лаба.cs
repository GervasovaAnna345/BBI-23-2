//В класс OneHundredMeterRunner добавить статическое поле "лучший результат". Сохранять и читать в отдельный файл значение поля OneHundredMeterRunner в фомате json
...
{
    public static double BestResult { get; set; }
...
System.IO.File.WriteAllText(Path.Combine(path, best_result.json), string.Empty);
mySerializers[i].Write<FiveHundredMeterRunner[]>(oneHundredMeterRunner, Path.Combine(path, best_result.json));
string bestResultJson = System.IO.File.ReadAllText(Path.Combine(path, "best_result.json"));
double bestResultFromJson = JsonSerializer.Deserialize<double>(bestResultJson);
Console.WriteLine($"Best Result: {bestResultFromJson}");